...
export const ADD_AVATAR = gql`
  mutation changeAvatar($id: uuid!, $avatar: String!) {
    update_userData(where: { id: { _eq: $id } }, _set: { avatar: $avatar }) {
      returning {
        id
      }
    }
...
export const SET_LIKE = gql`
  mutation setLikedPhoto($userID: uuid!, $postRef: uuid!) {
    insert_userLikes(objects: { userID: $userID, postRef: $postRef }) {
      returning {
        isLiked
      }
    }
...
export const DELETE_POST = gql`
  mutation deletePost($id: uuid!, $userID: uuid!) {
    delete_userPosts(where: { id: { _eq: $id }, userID: { _eq: $userID } }) {
      returning {
        userID
      }
    }
